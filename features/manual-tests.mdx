...
In **Gherkin** mode, the ADO steps are derived from your edited scenario:

- `Given` / `When` / `And` (before `Then`) → **Action** step.
- `Then` / `And` / `But` (after `Then`) → **Validation** step (`Verify:` in the action field, the line text as the expected result).

The full scenario is also written to `System.Description`.

<Warning>
  In Gherkin mode the push needs at least one recognisable `Given` / `When` / `Then` step. If the edited scenario has none, the push is **blocked** with an actionable message — there is no silent fallback to classic steps. Fix the scenario, then push.
</Warning>
